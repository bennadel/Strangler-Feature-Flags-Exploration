component
	output = false
	hint = "I provide an operator that tests to see if a value has one of the given prefixes."
	{
	...
	*/
	...
	/**
	* I test the given value to see if it starts with one of the set of target values. The
	...
	*/
	public boolean function testValue( required any value ) {
		...
		for ( var targetValue in targetValues ) {
			...
			if ( value.left( targetValue.len() ) == targetValue ) {
