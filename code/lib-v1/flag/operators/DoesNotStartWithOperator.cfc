component
	...
	output = false
	hint = "I provide an operator that tests to see if a value does not have one of the given prefixes."
	{
	...
	*/
	public void function init( required array targetValues ) {
		...
		variables.startsWithOperator = new StartsWithOperator( targetValues );
	...
	/**
	* I test the given value to see if it does not start with one of the set of target
	...
	*/
	public boolean function testValue( required any value ) {
		...
		return( ! startsWithOperator.testValue( value ) );
